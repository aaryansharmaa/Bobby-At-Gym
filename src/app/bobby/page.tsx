...
export default function BobbyPage() {
  ...
  // Function to toggle danger mode
  async function toggleDangerMode() {
    const newStatus = !isDangerMode;
    const success = await setDangerStatus(newStatus);
    if (success) {
      setIsDangerMode(newStatus);
    }
  }
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          {user && (
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`flex items-center text-sm px-4 py-2 rounded-full transition-colors font-medium shadow-sm ${
                  isDangerMode
                    ? "bg-red-600 hover:bg-red-700 text-white"
                    : "bg-gray-200 hover:bg-gray-300 text-gray-800"
                }`}
                title={
                  isDangerMode ? "Disable danger mode" : "Enable danger mode"
                }
              >
                <FiAlertTriangle
                  className={`mr-1 ${isDangerMode ? "animate-pulse" : ""}`}
                />
                {isDangerMode ? "Danger ON" : "Danger OFF"}
              </button>
        ...
        {isDangerMode && (
          <div className="bg-red-600 text-white p-3 flex items-center justify-center">
            <FiAlertTriangle className="mr-2 animate-pulse" size={20} />
        ...
        {!isDangerMode && user && (
          <div className="bg-blue-500 text-white p-3 flex items-center justify-center">
            ...
            <span className="font-medium">GYM IS CURRENTLY A SAFE SPACE</span>
          </div>
        )}
